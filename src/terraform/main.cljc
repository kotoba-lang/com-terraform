(ns terraform.main
  "Kotodama WASM entrypoint for the Terraform clean-room actor (L4) — Clojure port.
  ...
  facts (`terraform.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "terraform")
(def tier "L4")
...
(def entity-specs
  [{:entity "Run"   :plural "runs"   :id-prefix "terrafor_run"
    :fields [:type :status :message :source :triggerReason :hasChanges :autoApply :isDestroy :planOnly :createdAt :canceledAt] :required [:type]
    :coerce {:hasChanges :bool :autoApply :bool :isDestroy :bool :planOnly :bool} :refs {}}
   {:entity "Plan"      :plural "plans"      :id-prefix "terrafor_pla"
    :fields [:type :status :hasChanges :resourceAdditions :resourceChanges :resourceDestructions] :required [:type]
    :coerce {:hasChanges :bool :resourceAdditions :int :resourceChanges :int :resourceDestructions :int} :refs {}}
   {:entity "Apply"     :plural "applies"     :id-prefix "terrafor_app"
    :fields [:type :status :resourceAdditions :resourceChanges :resourceDestructions] :required [:type]
    :coerce {:resourceAdditions :int :resourceChanges :int :resourceDestructions :int} :refs {}}
   {:entity "Workspace" :plural "workspaces" :id-prefix "terrafor_wor"
    :fields [:name :type :description :executionMode :locked :resourceCount :autoApply :terraformVersion] :required [:name]
    :coerce {:locked :bool :resourceCount :int :autoApply :bool} :refs {}}
   {:entity "StateVersion" :plural "stateversions" :id-prefix "terrafor_sta"
    :fields [:status :serial :terraformVersion :resourcesProcessed] :required [:status]
    :coerce {:serial :int :resourcesProcessed :bool} :refs {}}
   {:entity "ConfigurationVersion" :plural "configurationversions" :id-prefix "terrafor_con"
    :fields [:type :status :source :speculative :provisional :autoQueueRuns] :required [:type]
    :coerce {:speculative :bool :provisional :bool :autoQueueRuns :bool} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"terraform.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "terraform-compat" :tier tier :entities entities} 200])
...
(defn start! [] :terraform-compat/ready)
